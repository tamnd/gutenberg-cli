// Package cli builds the gutenberg command tree on top of the gutenberg library.
package cli
...
type App struct {
	client *gutenberg.Client
	cfg    gutenberg.Config
...
func Root() *cobra.Command {
	app := &App{cfg: gutenberg.DefaultConfig()}

	root := &cobra.Command{
		Use:   "gutenberg",
		Short: "Browse Project Gutenberg free ebooks via Gutendex API",
		Long: `gutenberg reads Project Gutenberg through the open Gutendex community API
at https://gutendex.com. No API key or account required.

It returns records as table, JSON, JSONL, CSV, TSV, or URLs.

gutenberg is an independent tool and is not affiliated with Project Gutenberg
or the Gutenberg Literary Archive Foundation.`,
		SilenceUsage:  true,
		SilenceErrors: true,
...
func (a *App) setup() error {
	...
	a.client = gutenberg.NewClient(a.cfg)
	return nil
}
