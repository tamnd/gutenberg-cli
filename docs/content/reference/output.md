...
describes what users get. Pick a format with `-o`, or let gutenberg choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
gutenberg <command> -o table   # aligned columns for reading
gutenberg <command> -o jsonl   # one JSON object per line, for piping
gutenberg <command> -o json    # a single JSON array
gutenberg <command> -o csv     # spreadsheet friendly
gutenberg <command> -o tsv     # tab-separated
gutenberg <command> -o url     # just the URL column
gutenberg <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
gutenberg <command> --fields id,title,url
```
...
```bash
gutenberg <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
gutenberg <command>            # a table, because this is a terminal
gutenberg <command> | wc -l    # JSONL, because this is a pipe
```
